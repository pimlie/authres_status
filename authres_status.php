...
class authres_status extends rcube_plugin
{
    ...
    private static $RFC5451_authentication_methods = array(	"auth", 
    																												"dkim", 
    																												"domainkeys", 
    																												"sender-id", 
    																												"spf"
    																											);
    private static $RFC5451_authentication_results = array(	"none" => self::STATUS_NOSIG, 
    ...
    public function init()
    {
        ...
        } else if ($rcmail->action == 'list' || $rcmail->action == 'refresh') {
            $this->add_hook('storage_init', array($this, 'storage_init'));
	        	...
	        	if(preg_match('/('.implode("|", self::$RFC5451_authentication_methods).')'.$cfws.'='.$cfws.'('.implode("|", array_keys(self::$RFC5451_authentication_results)).')'.$cfws.'(\(.*?\))?/', $resinfo, $m, PREG_OFFSET_CAPTURE)) {
	        		...
	        		if($propspec) {
	        			if(preg_match_all('/('.implode("|", self::$RFC5451_ptypes).')'.$cfws.'\.'.$cfws.'('.implode("|", self::$RFC5451_properties).')'.$cfws.'='.$cfws.'([^\s]*)/i', $propspec, $m)) {
	        				foreach($m[0] As $k => $v) {
	        					if(!isset($parsed_resinfo['props'][$m[1][$k]])) $parsed_resinfo['props'][$m[1][$k]] = array();
